...
trench coat — and why adding a thirteenth algorithm is one Go file and zero
frontend changes.
...
```
  ALGORITHMS  ──emit──▶  TRACE (JSON)  ──read──▶  PLAYER ──▶ RENDERERS
   ...
   user Go   (Stage B)    + provenance            backward    call stack,
...
No server required: the twelve built-in algorithms ship as static JSON next to
...
| `conformance.sh` | 581 step hashes match across 12 traces, in two languages |
...
| | |
|---|---|
| **Stage A** | shipped — Go engine, 12 algorithms, 4 renderer families, CLI, conformance suite |
